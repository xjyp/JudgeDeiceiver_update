    ...
    def __init__(self,
        ...
        *args, **kwargs
        ...
        try:
            ...
            # InternLM chat templates use 'internlm-chat' or similar; treat them like 'llama-3'.
            if name_attr.startswith('internlm') or name_attr == 'internlm-chat':
                self.conv_template.name = 'llama-3'
            ...
            if self.conv_template.name == 'one_shot':
                self.conv_template.name = 'llama-3'
            # Treat zero_shot templates like llama-3 for prompt layout
            if self.conv_template.name == 'zero_shot':
                self.conv_template.name = 'llama-3'
    ...
    def _update_ids(self):
        ...
        try:
            ...
            if getattr(self.conv_template, 'name', '') == 'qwen-7b-chat':
                self.conv_template.name = 'qwen'
            if getattr(self.conv_template, 'name', '') == 'guanaco':
                self.conv_template.name = 'qwen'
            # Accept zero_shot as an alias and treat like llama-3
            if getattr(self.conv_template, 'name', '') == 'zero_shot':
                self.conv_template.name = 'llama-3'
            if 'vicuna' in lname:
                self.conv_template.name = 'llama-3'
            if 'guanaco' in lname:
            ...
            if getattr(self.conv_template, 'name', '').startswith('gemma'):
                self.conv_template.name = 'qwen'
            # Accept InternLM chat-style templates by mapping them to 'llama-3'
            if getattr(self.conv_template, 'name', '').startswith('internlm') or getattr(self.conv_template, 'name', '') == 'internlm-chat':
                self.conv_template.name = 'llama-3'
        except Exception:
            pass
...
class ModelWorker(object):
    ...
    def __init__(self, model_path, model_kwargs, tokenizer, conv_template, device):
        ...
        try:
            ...
            #         trust_remote_code=True
            # )
            # 2) VL 模型类，替代 AutoModelForCausalLM
            #    dtype="auto" / torch.float16 / torch.bfloat16 都可以，按你显存情况选
            self.model = Qwen3VLForConditionalGeneration.from_pretrained(
                self.model_path,
                torch_dtype=torch.float16,     # 或 torch.bfloat16 / dtype="auto"
                device_map={"": 0},            # 强制放到第一个 GPU（和你原来一致）
                attn_implementation="sdpa",    # 更稳；若装了 flash-attn2 可改为 "flash_attention_2"
            )
...
def _normalize_model_path(p):
    # list/tuple: 取第一个
    if isinstance(p, (list, tuple)):
        return str(p[0]).strip()

    # 字符串但长得像 "['/path']"：转成 list 再取第一个
    if isinstance(p, str):
        s = p.strip()
...
def get_workers(params, eval=False):
    ...
    for i in range(len(params.tokenizer_paths)):
        ...
        # )
        # 1) Processor（多模态：图像+文本），替代 AutoTokenizer
        model_path = _normalize_model_path(params.model_paths)
        ...
        model.eval()
        print("✅ Qwen3-VL-8B-Instruct model loaded successfully")
...
def get_target(data, params, intro, texts1, texts2):
    ...
    print("🔍 没有 target 字段，开始通过 VLLMJudge 解析 text 获取 preference...")
    print(getattr(params, 'judge_model', None))
    judge = create_vllm_judge(model_path=getattr(params, 'judge_model', None) or "/root/autodl-tmp/Qwen3-VL-8B-Instruct/",dtype="bfloat16")
    qids = getter('question_id', [""] * n) if callable(getter) else getattr(data, 'question_id', [""] * n)
    model_as = getter('model_a', [""] * n) if callable(getter) else getattr(data, 'model_a', [""] * n)
    model_bs = getter('model_b', [""] * n) if callable(getter) else getattr(data, 'model_b', [""] * n)
    qids = _to_list(qids, n)

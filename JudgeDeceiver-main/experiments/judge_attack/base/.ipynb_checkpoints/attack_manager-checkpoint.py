    ...
    try:
        ...
        try:
            if torch.is_tensor(ids):
                ...
                ids_list = list(ids)
            toks = tokenizer.convert_ids_to_tokens(ids_list, skip_special_tokens=skip_special_tokens)
...
def get_goals_and_targets(params):
    ...
    if params.train_data:
        ...
        train_intro = train_data.get('instruction', [""] * len(train_data)).tolist()
        train_texts1 = train_data.get('response_a', [""] * len(train_data)).tolist()
        train_texts2 = train_data.get('response_b', [""] * len(train_data)).tolist()
        # --- Step 5. 处理 target ---
        train_targets = get_target(train_data,params,train_intro,train_texts1,train_texts2)

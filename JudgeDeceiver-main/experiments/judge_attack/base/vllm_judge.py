...
# Reply with only one letter: A, B, or C."""
    ...
    def judge_examples(
        ...
        sparams = SamplingParams(
            max_tokens=min(max_new_tokens or self.sparams.max_tokens, 20),
        ...
        # 按 batch_size 分块调用，避免一次性占用过多显存
        for i in range(0, len(prompts), batch_size):
            batch_prompts = prompts[i : i + batch_size]
            results = self.llm.generate(batch_prompts, sparams)
            # print("🔹 Model outputs (batch):", results)
            for j, r in enumerate(results):
                text = r.outputs[0].text if r and r.outputs else ""
                # 传入对应的 prompt，保持与单样本路径一致
                pref, conf = self._parse_response(text, batch_prompts[j])

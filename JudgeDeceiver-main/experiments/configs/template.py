...
def get_config():
    ...
    # tokenizers
    # "/share/disk/llm_cache/Qwen2.5-7B-Instruct"
    # config.tokenizer_paths=['/share/disk/llm_cache/Mistral-7B-Instruct-v0.3']
    config.tokenizer_paths=['/share/disk/llm_cache/Qwen2.5-7B-Instruct']
    config.tokenizer_kwargs=[{"use_fast": False, "local_files_only": True}]

    config.model_paths=['/share/disk/llm_cache/Qwen2.5-7B-Instruct']
    ...
    #eval parameters
    config.eval_model = 'qwen25'
    # optional: judge model used by vllm judge (can be HF repo id or local path)
    # 使用空字符串作为默认值以便支持命令行覆盖（ml_collections 只支持基础类型）
    config.judge_model = ''
